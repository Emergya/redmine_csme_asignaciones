	...
	function getProviders(data){
		...
		if(data.providers.length > 0 ){
			...
			for(var i=0; i<data.providers.length; i++){
				...
				//if(data.providers[i].gg_article.guarantee_end < new Date()){
				//	$content_providers += "<tr class='guarantee_end_out'>";
				//}else{
					$content_providers += "<tr class='test'>";
				//}
	...
	$(document).on("click", "#btn_form_article_csme", function(){
		...
		// Petición para obtener articulos de la table ISE_MATERIAL_DISTRIBUIDO_GARANTIAS.
   		$.ajax({
			...
			type: "GET",
			data: { cod_file: cod_file, cod_article: cod_article, cod_center: cod_center, cod_adj: cod_adj },
